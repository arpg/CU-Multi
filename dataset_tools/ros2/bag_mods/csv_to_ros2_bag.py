...
SUBSAMPLE_N = 5 # write odom/path every third CSV row
...
# ==================== Create BAG ====================
def create_ros2_pose_bag(data_root, env, robot):
    bag_dir = os.path.join(data_root, f"{env}/robot{robot}")
    csv_file = os.path.join(bag_dir, f"robot{robot}_{env}_gt_utm_poses.csv")
    out_uri = os.path.join(bag_dir, f"robot{robot}_{env}_gt_rel_poses")
    ...
    IMU_TO_LIDAR_Q_NORMALIZED = q_normalize(IMU_TO_LIDAR_Q)
    p_map = origin_pos_imu + q_rotate_vec(origin_quat_imu, IMU_TO_LIDAR_T)
    q_map = q_multiply(origin_quat_imu, IMU_TO_LIDAR_Q_NORMALIZED)
...
if __name__ == "__main__":
    ...
    for env in environments:
        for robot in robots:
            create_ros2_pose_bag(data_root, env, robot)
